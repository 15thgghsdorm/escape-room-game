...
function updatePenaltyDisplay() {
  ...
  document.getElementById('penalty-display').textContent = `패널티: +${penaltySeconds}초`;
}
...
socket.on('password-correct', (data) => {
  ...
  const penaltySeconds = Math.floor(penaltyTime / 1000);
  document.getElementById('final-penalty').textContent = 
    `패널티: +${penaltySeconds}초`;
...
function requestHint() {
  ...
  document.getElementById('hint-request-text').value = '';
  document.getElementById('hint-request-text').focus();
}
...
function sendHintRequest() {
  ...
  document.getElementById('hint-request-popup').classList.add('hidden');
