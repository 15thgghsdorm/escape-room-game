...
function updateTimer(roomNumber, roomData) {
    if (!currentGameState || !currentGameState.rooms) return;
    
    const currentRoomData = currentGameState.rooms[roomNumber];
    if (!currentRoomData || !currentRoomData.isActive || currentRoomData.completed) {
        if (roomTimers[roomNumber]) {
            clearInterval(roomTimers[roomNumber]);
            delete roomTimers[roomNumber];
        }
    ...
    const now = Date.now();
    const elapsed = now - currentRoomData.startTime;
    const penaltyTime = currentRoomData.penalties * 30000;
    const totalTime = elapsed + penaltyTime;
